$(document)
  ...
  .on("click", function () {
    ...
    $("#com-pkg-sms-tab .com-pkg-individual-tab-content:first").addClass(
      "active"
    );

    $("#com-pkg-Telephoney-tab .com-pkg-tab-item:first").addClass("active");
    $("#com-pkg-Telephoney-tab .com-pkg-individual-tab-content:first").addClass(
      "active"
    );

    $("#com-pkg-video-call-tab .com-pkg-tab-item:first").addClass("active");
    $("#com-pkg-video-call-tab .com-pkg-individual-tab-content:first").addClass(
      "active"
    );
...
let session_available = localStorage.getItem("session_available");

console.log("session_available", session_available);

if (!session_available) {
  $("#com-pkg-audio-call").attr("disabled");
  $("#com-pkg-audio-call").css("opacity", "0.6");
  $("#com-pkg-audio-call").css("cursor", "not-allowed");
}
...
let telephonyInboundTable = $("#com-pkg-telephony-incoming-table")
  ...
  .DataTable({
    ...
    columns: [
      { data: "id", title: "ID" },
      { data: "destination_number", title: "To" },
      { data: "call_record_url", title: "Recording Link" },
      { data: "call_type", title: "Type" },
      { data: "agentId", title: "Agent ID" },
      { data: "start_time", title: "Time" },
      { data: "call_duration", title: "Call Duration" },
    ],
    ...
    ajax: function (d, callback) {
      ...
      fetch(
        `${baseUrl}/api/?action=fetch_inbound_call_records&${new URLSearchParams(flattenedObject)}`,
        {
          method: "GET",
          headers: {
    ...
    dom: '<"table-top" ><"table-search-box-telephony-incoming table-search-box">t<"table-bottom" <"table-info" i> <"table-pagination-box" l<"table-pagination-box-divider">p> >',
    initComplete: function () {
      let totalColumns = this.api().columns().count();
      telephonyInboundTable.columns.adjust().draw();
      $(".table-search-box-telephony-incoming").html(`
                    <div class="table-search-input-box com-pkg-search-box">
                        ...
                        <input id='table_search_incoming' type="text" placeholder="Search"/>
                    </div>
                    ${
      ...
      function searchInTable(value) {
        telephonyInboundTable.search(value).draw();
      }
      const searchFieldChange = debounce((value) => searchInTable(value), 500);

      $(document)
        .find("#table_search_incoming")
      ...
      new $.fn.dataTable.FixedHeader(telephonyInboundTable, {
        headerOffset: $(".table-search-box-telephony-incoming").outerHeight(),
        header: true,
        footer: false,
      });
...
let telephonyOutgoingTable = $("#com-pkg-telephony-outgoing-table")
  .on("processing.dt", function (e, settings, processing) {
    $(document).find(".overlay-spinner").hide();
    onTableProcessing(processing);
  })
  .DataTable({
    ...
    ajax: function (d, callback) {
      ...
      fetch(
        `${baseUrl}/api/?action=fetch_outbound_call_records&${new URLSearchParams(flattenedObject)}`,
        {
          method: "GET",
          headers: {
            "X-CSRFToken": "{{csrf_token|safe}}",
    ...
    dom: '<"table-top" ><"table-search-box-telephony-outgoing table-search-box">t<"table-bottom" <"table-info" i> <"table-pagination-box" l<"table-pagination-box-divider">p> >',
    initComplete: function () {
      let totalColumns = this.api().columns().count();
  ...
  $("#new-call-btn").on("click", async function (event) {
    ZelthyRJSF(".zelthy-dynamic-form", {
      ...
      post_data: {
        http_method: "POST",
        url: `${window.location.origin}/communication/sms/api/?action=send`,
...
$("#com-pkg-audio-call").on("click", async function () {
  ...
  if (localStorage.getItem("session_available")) {
